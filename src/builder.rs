...
use config::{Config, Operation, RemoveOptions, SyncOptions};
use error::*;
use message::{BuilderPackage, Message};
...
fn sync(
	...
	for package in &opts.packages {
		let package =
			match util::sync_package(cfg, opts, &package.name) {
				Err(e) => {
					error!("{}", e);
					continue;
				},
		...
		err!(
			tx.send(Message::PackageBuilt(package)),
...
fn remove(opts: &RemoveOptions) -> Result<()> {
	for package in &opts.packages {
		err!(
...
fn main() {
	...
	let cfg = match get_config(&mut c) {
		Ok(v) => v,
		Err(e) => {
	...
	let res = match &cfg.operation {
		Operation::Sync(ref v) => sync(&mut c, &cfg, &v),
		Operation::Remove(ref v) => remove(&v),
...
mod util {
	...
	pub fn sync_package(
		...
		opts: &SyncOptions,
		package_name: &str,
	) -> Result<BuilderPackage> {
		let package_dir = Path::new(package_name);

		if !package_dir.exists() {
			err!(
				clone_package(&cfg.aur, package_name),
