...
pub struct SyncOptions {
	...
	#[clap(short, long, help = "Install packages after build")]
	pub install: bool,
...
/*
pub fn operation(subcommand: (&str, &clap::ArgMatches)) -> Operation {
	let (command, args) = subcommand;

	match command {
		"sync" => Operation::Sync(SyncOptions {
			upgrade: args.is_present("upgrade"),
			install: args.is_present("install"),
			build_args: args.values_of_t("buildargs").unwrap(),
			packages: args
				.values_of_t("packages")
				.unwrap_or_default(),
		}),
		"remove" => Operation::Remove(RemoveOptions {
			uninstall: args.is_present("uninstall"),
			packages: args
				.values_of_t("packages")
				.unwrap_or_default(),
		}),
		"build" => Operation::Build(BuildOptions {
			machine_image: args.value_of("image").unwrap().to_owned(),
		}),
		"query" => Operation::Query(QueryOptions {
			info: args.is_present("info"),
			keywords: args
				.values_of_t("keywords")
				.unwrap_or_default(),
			by: args.get_one::<aur::By>("by").unwrap().clone(),
			output: args
				.get_one::<aur::Output>("output")
				.unwrap()
				.clone(),
		}),
		"completions" => Operation::Completions(CompletionOptions {
			shell: args.value_of("shell").map(|x| x.to_owned()),
		}),
		"runtime" => Operation::Runtime(RuntimeOptions {
