...
complete -c zeus -n "__fish_seen_subcommand_from sync" -s i -l install -d 'Install packages after build'
...
complete -c zeus -n "__fish_seen_subcommand_from build" -l image -d 'Builder machine image' -r
